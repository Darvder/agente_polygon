"""
generar_dashboard_momentum.py — v3.1 (Fixed)
Mejoras:
  - Corrección de SyntaxError en f-strings (línea de Exit Stats)
  - Uniformidad en el uso de triple comillas para bloques HTML
"""
...
def generar():
  ...
  {bar}
    ...
    for t, label, ac in [("TAKE_PROFIT","TP ✅","#10b981"),
        ...
        # Corrección: Uso de comillas triples para evitar SyntaxError con condicionales internos
        if t == "TIME_EXIT":
...
gap:10px;align-items:center;padding:8px 4px;border-bottom:1px solid #0d1829">
  ...
  <span style="color:{col};font-weight:700;font-size:13px;text-align:right;font-family:'Space Mono',monospace">{pnl:+.2f}$</span>
    ...
    # [Resto del código del dashboard HTML permanece igual pero verificado]
    html = f"""<!DOCTYPE html>
<html lang="es"><head>
...
if (values.length > 0) {{
  ...
  document.getElementById('chart').parentElement.innerHTML =
    ...
    print(f"✅ Dashboard v3.1 → {ARCHIVO_OUT}")
